          ...
          >
            <option value='All'>All</option>
            {
              categories && categories.slice(1, 4).map((category, index) => (
                <option value={category.seasonCategoryName} key={index}>{category.seasonCategoryName}</option>
              ))
            }								
          </select>
        </div>
            ...
            <ul className='dao-proposal-content'>
              {
                ...
                >
                  <div>
                    ...
                    <p className={`${list.status === 'Active' ? 'active' : 'closed'}`}>
                      {list.status}
                    </p>
                  </div>
                  <h2>{list.title}</h2>
